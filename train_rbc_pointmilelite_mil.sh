...
choices=(2)  # Replace with actual fold numbers if different
poolings=("gap")  # Replace with actual pooling options

# Original YAML file
config_file="cfgs/modelnet40ply2048/pointmil_pointmlpelite_rbc.yaml"
...
for choice in "${choices[@]}"; do
  for pooling in "${poolings[@]}"; do
    ...
    modified_config="${output_dir}/pointmil_pointmlpelite_rbc_choice_${choice}_pooling_${pooling}.yaml"
    ...
    run_log="${output_dir}/pointmil_pointmlpelite_rbc_choice_${choice}_pooling_${pooling}.log"
    run_dir="${output_dir}/pointmil_pointmlpelite_rbc_choice_${choice}_pooling_${pooling}"
    mkdir -p "$run_dir"
