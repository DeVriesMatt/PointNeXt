...
choices=(3 4)  # Replace with actual fold numbers if different
poolings=("None")  # Replace with actual pooling options

# Original YAML file
config_file="cfgs/modelnet40ply2048/pointnext-s_nodown_rbc.yaml"
...
for choice in "${choices[@]}"; do
  for pooling in "${poolings[@]}"; do
    ...
    modified_config="${output_dir}/config_choice_${choice}_pooling_${pooling}.yaml"
    ...
    run_log="${output_dir}/run_choice_${choice}_pooling_${pooling}.log"
    run_dir="${output_dir}/run_choice_${choice}_pooling_${pooling}"
    mkdir -p "$run_dir"
