...
choices=(0 1 2 3 4)  # Replace with actual fold numbers if different
poolings=("additive" "conjunctive" "instance" "attention" "gap")  # Replace with actual pooling options

# Original YAML file
config_file="cfgs/modelnet40ply2048/pointnext-s_nodown_intra.yaml"
output_dir="cfgs/modelnet40ply2048/"
...
for pooling in "${poolings[@]}"; do
  for choice in "${choices[@]}"; do
    ...
    modified_config="${output_dir}/new_intra_config_choice_${choice}_pooling_${pooling}.yaml"
    ...
    run_log="${output_dir}/new_intra_run_choice_${choice}_pooling_${pooling}.log"
    run_dir="${output_dir}/new_intra_run_choice_${choice}_pooling_${pooling}"
    mkdir -p "$run_dir"
    ...
    echo "Training completed for choice=$choice and pooling=$pooling. Results saved to $run_dir"
  done
done
