...
choices=(1)  # Replace with actual fold numbers if different
poolings=("gap")  # Replace with actual pooling options

# Original YAML file
config_file="cfgs/modelnet40ply2048/pointmil_pointmlpelite_intra.yaml"
...
for pooling in "${poolings[@]}"; do
  for choice in "${choices[@]}"; do
    ...
    modified_config="${output_dir}/mlp_mil_intra_choice_${choice}_pooling_${pooling}.yaml"
    ...
    run_log="${output_dir}/mlp_mil_intra_run_choice_${choice}_pooling_${pooling}.log"
    run_dir="${output_dir}/mlp_mil_intra_run_choice_${choice}_pooling_${pooling}"
    mkdir -p "$run_dir"
